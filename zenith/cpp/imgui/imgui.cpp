    ...
    if (g.NavWindow)
        if (ImGuiWindow* focused_root_window = g.NavWindow->RootWindow)
            if (focused_root_window->WasActive && focused_root_window != window->RootWindow)
            {
                ...
                // NB: The order of those two test is important because Modal windows are also Popups.
...
static void FindHoveredWindow()
{
    ...
    for (int i = g.Windows.Size - 1; i >= 0; i--)
    {
        ...
        // Those seemingly unnecessary extra test are because the code here is a little different in viewport/docking branches.
...
void ImGui::EndGroup()
{
    ...
    // (The test not symmetrical because ActiveIdIsAlive is an ID itself, in order to be able to handle ActiveId being overwritten during the frame.)
